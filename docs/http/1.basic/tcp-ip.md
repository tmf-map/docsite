...
由于TCP/IP 等协议已经在许多网络上实际运行，所以将他们全部推翻重构显然是不现实的。此外，OSI的分层模型在四层以上分的太细，而 TCP/IP实际应用时的会话管理、编码转换、压缩等和具体应用经常联系的很紧密，很难分开。例如，HTTP 协议就同时包含了连接管理和数据格式定义。因此，OSI分层模型在发布的时候就明确地表明只是作为一个参考。

<Hint type="tip">OSI分层模型在发布的时候就明确地表明只是作为一个**参考**</Hint>
