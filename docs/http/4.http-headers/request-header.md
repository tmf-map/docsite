...
除了上述字段外还有`Origin`、`Access-Control-Request-Methods`、`Access-Control-Request-Headers`等字段，这些`Access-Control-Request-*`用在`CORS`跨域的预检请求报文中。关于这部分的内容可以参考文档中关于[CORS](/docs/web/1.cross-domain/cors)。
