...
以上代码每轮循环也是基于不断更新 `lastIndex` 属性工作的，`lastIndex` 只对同一个正则表达式有效，所以下面这样写是错误的：
