...
`path`字段的配置在`生产环境`下是必须的，因为我们需要为`webpack`指明打包路，但是在`开发环境`下不是必须的，因为当使用 `webpackDevServer`时打包出来的**文件都在内存中而没有打包到磁盘**，即使指定了输出目录，执行命令行后，输出目录也是空的。
...
1. 在生产环境中，通过设置`publicPath`添加 CDN 前缀，将我们的静态资源进行 CDN 托管。
2. 在开发环境中，通过设置`publicPath`添加相对路径前缀，在开启`webpackDevServer`时，指明打包后的资源路径，默认将其挡在项目的根目录。
...
下面我们结合 demo，着重讲解在开发环境中`publicPath`的表现。
...
<Img width="700" src="https://cosmos-x.oss-cn-hangzhou.aliyuncs.com/20200423180630.gif" />
...
<Img width="700" src="https://cosmos-x.oss-cn-hangzhou.aliyuncs.com/20200423181421.gif" />

通过图中可以看出，所有的静态资源都是以`/dist/`为前缀。

### devServer.publicPath

在开启 webpackDevServer 时浏览器中可通过该路径访问 `bundled` 文件，**静态文件前会加上这个路径前缀**。`devServer.publicPath`与`out.publicPath`之间的联系如下：

1. 若`devServer.publicPath`没有设置，则默认为`out.publicPath`的值。
2. 若`out.publicPath`的值没有设置，则所有静态文件以`devServer.publicPath`值为前缀。
3. 若两者都有设置且不相同，则使用`loader`打包出的静态文件以`out.publicPath`的值为前缀，`html`文件以`devServer.publicPath`为前缀。
4. 若两者都不设置的话，那所有的静态文件都以“/”为前缀。
...
上述结论的`1`和`4`在前面的案例中已经涉及，此时不再细述，下面重点说下`2`和`3`。
...
<Img width="700" src="https://cosmos-x.oss-cn-hangzhou.aliyuncs.com/20200423221610.gif" />

从打包结果可以看出，所有的文件都是以"/dist/"为前缀。

为了验证结论第 3 条，我们修改`output.publicPath`的值为`/test/`，打包结果如下：

<Img width="700" src="https://cosmos-x.oss-cn-hangzhou.aliyuncs.com/20200423222102.gif" />

通过打包过程可以看出，`search.html`以`devServer.publicPath`值为前缀，而其对应的`index.js`却以`output.publicPath`值为前缀。初次之外打包的页面也没能正常现实，因此在设置这两个值的时候，最好两者保持一致，或者只设置`output.publicPath`即可。
...
为了证明首页目录真的是`./dist`，我们可以在`./dist`新建一个文件`i-am-dist.html`，打包结果如下图所示：
