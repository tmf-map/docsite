...
以下内容以 SQLite 数据库为例讲解 SQLAlchemy 的基本用法，更详细的使用请阅读[SQLAlchemy 官方文档](https://docs.sqlalchemy.org/en/14/orm/quickstart.html)。
...
Python 安装包中集成了 SQLite 数据库的驱动，SQLite 是一个轻量级关系型数据库，实现了自给自足、无服务器的、零配置的、事务性的 SQL 数据库引擎，常用在移动端中。
...
这些类型与关系型数据库中的字段类型具有对应关系，比如 String 对应的数据库类型时 varchar，Numeric 对应的数据库类型是 float 或 decimal，详细使用请参阅[官方文档](https://docs.sqlalchemy.org/en/14/core/type_basics.html#generic-camelcase-types)。
...
class User(Base):
    ...
    account = relationship('Account', uselist=False, backref='account') # 第一参数是数据模型类名
```
